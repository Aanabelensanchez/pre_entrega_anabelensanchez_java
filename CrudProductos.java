...
public class CrudProductos extends CrudConsola<Producto> {
    ...
    // Primero -> si quiero actualizar algo y no hay productos cargados, que diga que debemos cargar algo para modificar
     if ( productos==null  || productos.isEmpty())
     {System.out.println("No hay productos cargados! Cargá un producto primero");
      return;
    }
    // Agrege la modificacion que cuando seleccione actualizar ya me liste todos los productos, para saber cual es el id que quiero actualizar, sino tendria que listar priemro y despues actualizar.
    System.out.println("=== Lista de productos ===");
    for (Producto p : productos) {
        String tipo = (p instanceof Articulo) ? "Artículo" : "Servicio";
        System.out.println("ID: " + p.getId() +
                           " | Nombre: " + p.getNombre() +
                           " | Precio: " + p.getPrecio() +
                           " | Tipo: " + tipo);
    }
        int id = leerEntero("Id del producto que queres modificar: ");
        for (Producto p : productos) {
            if (p.getId() == id) {
